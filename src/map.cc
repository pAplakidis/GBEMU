...
uint16_t* Range::contains(uint16_t addr) const{
    ...
    if((start == end && addr == start) || (addr >= start && addr < end)){
...
const Range *ROM = new Range(0x0000, 0x7fff);
...
const Range *WRAM = new Range(0xc000, 0xdfff);
const Range *ECHO_RAM = new Range(0xe000, 0xfdff);
//const Range *OAM = new Range(0xf000, 0xfe9f);
const Range *OAM = new Range(0xfe00, 0xfe9f);
const Range *UNUSABLE = new Range(0xfea0, 0xfeff);
const Range *IO_Regs = new Range(0xff00, 0xff7f);
const Range *HRAM = new Range(0xff80, 0xfffe);
